...
A new R package name `altRnative` is created to provide functionalities to do benchmark for R script across different Docker image [@altRnative2020]. `altRnative` uses `microbenchmark` for running the benchmark process and `stevedore` for managing the Docker image and containers. This package is available in the GitHub repository, in the following link: `https://github.com/ismailsunni/altRnative`. This package is tested to do a benchmark on Spatial Data Science with R Book. To run a benchmark, an R script is needed as the input, and also the list of combination between platform and R implementation.
