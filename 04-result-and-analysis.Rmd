...
In this section, the exploration's result of each combination between R implementations and platforms for the R installation and geospatial packages installation are shown. The result is described per each R implementations. The overview of the result can be seen in figure \@ref(tab:vanillaRDockerImagesTable) and \@ref(tab:geospatialRDockerImagesTable). There are some symbol in those two figures that can be explained using the legend in figure \@ref(fig:docker-images-legend)

```{r loadLibrary, included=FALSE, echo=FALSE, results='hide', warning=FALSE, message=FALSE}
library('kableExtra')
library('microbenchmark')
library('altRnative')
```

```{r vanillaRDockerImagesTable, results="asis", echo=FALSE}
...
vanillaRDockerImages %>%
  kable(col.names = c('GNU R', 'MRO', 'Renjin', 'FastR', 'pqR', 'TERR'), caption = "Vanilla R Docker Images") %>%
  kable_styling(full_width = T, latex_options = "striped") %>%
  row_spec(0, bold = T) %>%
  column_spec(1, bold = T)
```

```{r geospatialRDockerImagesTable, results="asis", echo=FALSE}
...
geospatialRDockerImages %>%
  kable(col.names = c('GNU R', 'MRO', 'Renjin', 'FastR', 'pqR', 'TERR'), caption = "Geospatial R Docker Images") %>%
  kable_styling(full_width = T, latex_options = "striped") %>%
  row_spec(0, bold = T) %>%
  column_spec(1, bold = T, width = '100em')
```
