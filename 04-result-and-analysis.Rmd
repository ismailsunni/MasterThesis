...
A new R package name `altRnative` is created to provide functionalities to do benchmark for R script accross different docker image. `altRnative` uses `microbenchmark` for running the benchmark process and `stevedero` for managing the docker image and containers. This package is available in GitHub repository, in the following link: `https://github.com/ismailsunni/altRnative`. This package is tested to do benchmark on Spatial Data Science with R Book. Currently, only successful geospatial docker are supported in this package. Below is the list of functionalities of `altRnative`:

1. Run R script inside a docker image
2. Run benchmark for R script across docker image
3. Create box plot from the benchmark result
4. Create a ratio based the mean value to see the ratio between the benchmark result
5. Create box plot from the benchmark ratio
